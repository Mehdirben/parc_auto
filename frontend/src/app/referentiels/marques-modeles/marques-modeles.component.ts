...
type Suppression =
  | { type: "marque"; code: string; libelle: string }
  | { type: "modele"; id: string; libelle: string };
...
export class MarquesModelesComponent implements OnInit, OnDestroy {
  ...
  ouvrirAjoutModele(): void {
    ...
    this.modeleForm.controls.nom.setValidators([
      Validators.required,
  ...
  enregistrerModele(): void {
    ...
    this.enregistrement = true;
    const requête: Observable<MarqueDetail | Modele> =
      this.modeleDialogMode === "create"
    ...
    requête.subscribe({
      next: (résultat) => {
        ...
        if ("modeles" in résultat) {
          ...
          this.selection = {
            ...this.selection,
            modeles: this.selection.modeles.map((item) =>
              item.id === résultat.id ? résultat : item,
            ),
          };
        }
        this.toast.show(
  ...
  confirmerSuppression(): void {
    ...
    const requête =
      cible.type === "marque"
        ...
        : this.service.supprimerModèle(cible.id);
    requête.subscribe({
      next: () => {
        this.enregistrement = false;
        ...
        if (cible.type === "modele" && this.selection) {
          this.selection = {
            ...this.selection,
            modeles: this.selection.modeles.filter(
        ...
        this.toast.show(
          "success",
          cible.type === "marque" ? "Marque supprimée" : "Modèle supprimé",
        );
        this.charger(this.page.page);
      },
      error: (erreur) => {
  ...
  identifierMarque(_index: number, marque: MarqueListe): string {
    return marque.code;
  }
  identifierModele(_index: number, modèle: Modele): string {
    return modèle.id;
  }
    ...
    return [
      ...new Map(
        valeur
          ...
          .map((nom) => nom.trim())
          .filter(Boolean)
          .map((nom) => [nom.toLocaleUpperCase("fr"), nom]),
      ).values(),
    ];
  }
